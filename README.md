...
개인과제는 미션1(PoC 4종, Vibe Coding)과 미션2(본 리포지토리, 엄격한 Agentic Engineering)로
구성되며, 이 리포지토리는 **미션2 최종 프로젝트**다. 미션1의 각 PoC는 최종 프로젝트에 들어갈
핵심 패턴을 미리 검증하는 역할을 했고, 검증된 패턴은 본 프로젝트의 `app/` 아래로 그대로
계승되었다(§관련 리포지토리 참고). 미션1은 빠른 검증이 목적이라 AE 절차(설계 문서 사전 승인 등)를
따르지 않았고, 미션2인 본 리포지토리만 Human-in-the-loop 기반 AE(설계→승인→구현→검증→커밋)를
전 과정에 적용했다.

```
[미션1: PoC 4종, Vibe Coding]                    [미션2: 최종 프로젝트 - 본 리포지토리]
ConsoleMVC          (MVC 계층 분리 검증)     -->  app/models, app/views, app/controllers
DataPersistence      (JSON CRUD 영속성 검증) -->  app/repositories/json_store.py
DataMonitor          (모니터링 집계 검증)    -->  app/controllers/monitoring_controller.py
DummyDataGenerator   (더미 데이터 생성 검증) -->  1000건 규모 성능 스트레스 테스트에 활용
```
...
> 아래 화면들은 `python main.py` 실행 결과를 그대로 캡처한 것이다(가공 없이 복사, `>` 뒤가 입력값).
...
============================================================
 반도체 시료 생산주문관리 시스템
============================================================
 시스템 현황  등록 시료 2종  총 재고 510ea  전체 주문 2건  생산라인 0건 대기
------------------------------------------------------------
...
선택 > 주문번호 > 승인 완료. 상태: CONFIRMED
------------------------------------------------------------
 [주문 승인/거절]
[1] 승인   [2] 거절   [0] 뒤로
주문번호        시료ID  고객명              수량    상태
ORD-0002        S-003   삼성전자 파운드리   200     [RESERVED]
선택 > 주문번호 > 승인 완료. 상태: PRODUCING
```
...
선택 > 순서  주문번호        시료ID  부족분  주문수량
1     ORD-0002        S-003   170     200
------------------------------------------------------------
 [생산 라인]
[1] 다음 항목 생산 처리   [2] 대기열 조회   [0] 뒤로
선택 > 생산 완료 - 주문 ORD-0002 / 시료 S-003 / 부족분 170 -> 실생산량 185 (예상 소요 148.00분)
```
...
각 PoC가 검증한 패턴이 본 프로젝트 어디로 계승되었는지는 위 "과제 전체 구성" 표를 참고.

- [ConsoleMVC-jihoonlee-23020975](https://github.com/jihoonlee91/ConsoleMVC-jihoonlee-23020975) — MVC 계층 분리(Model/View/Controller) 검증
- [DataPersistence-jihoonlee-23020975](https://github.com/jihoonlee91/DataPersistence-jihoonlee-23020975) — JSON 기반 CRUD 영속성(`JsonStore`) 검증
- [DataMonitor-jihoonlee-23020975](https://github.com/jihoonlee91/DataMonitor-jihoonlee-23020975) — 주문/재고 모니터링 집계 로직 검증
- [DummyDataGenerator-jihoonlee-23020975](https://github.com/jihoonlee91/DummyDataGenerator-jihoonlee-23020975) — 참조 무결성을 지키는 더미 데이터 생성 검증
