...
Phase 11부터 `.github/workflows/test.yml`로 push/PR 시 pytest를 원격에서도 자동 실행한다.
이는 로컬 Test Verify를 대체하는 것이 아니라, "커밋 전 사람이 직접 실행해야만 검증된다"는
빈틈을 메우는 추가 안전망이다.
...
- `JsonStore`는 인메모리 캐시로 "읽기"는 빠르지만, `insert`/`update`/`delete`는 기본적으로
  매번 파일 전체를 다시 직렬화(write)한다(Phase 11에서 `flush=False` + `flush()`로 배치 쓰기를
  선택적으로 지원하도록 개선했으나, 인터랙티브 콘솔 흐름은 매번 즉시 flush하므로 "1건씩
  승인/등록"하는 실사용 동작 자체는 변하지 않는다). 대량 삽입/배치 시나리오(예: 더미 데이터
  일괄 생성)에서는 `flush=False`로 호출 후 마지막에 `flush()` 한 번만 호출하면 유의미하게
  빨라진다(1000건 배치 삽입 기준 1.51s → 약 0.005s, `docs/REPORT.md` Phase 11 참고). 승인/생산
  처리처럼 "매 건마다 즉시 영속화가 안전을 위해 필요한" 흐름은 여전히 매번 flush하며, 이는
  의도적 설계 결정이다(반도체 시료 주문관리 실사용 규모에서는 문제되지 않음).
