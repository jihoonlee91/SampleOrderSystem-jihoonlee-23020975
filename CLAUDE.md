...
- `JsonStore`는 인메모리 캐시로 "읽기"는 빠르지만, `insert`/`update`/`delete`는 매번 파일
  전체를 다시 직렬화(write)한다. 주문/시료가 수백~1000건 이상 누적되면 승인·생산 처리 같은
  쓰기 작업이 선형보다 느리게 체감될 수 있다(1000건 규모 스트레스 테스트 기준 승인 1000건에
  약 18초). 실사용 규모(반도체 시료 주문관리, 일반적으로 수십~수백 건)에서는 문제되지 않는
  수준이라 판단하여 배치 쓰기 등 추가 최적화는 진행하지 않았다(의도적 범위 제한).
