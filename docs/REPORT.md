...
## Phase 10: JsonStore 인메모리 캐싱 (성능 최적화)

### 요청 받은 작업
스트레스 테스트(주문 1000건)에서 발견된 성능 문제(승인 1000건에 17.8초)를 설계 문서
(docs/design/phase10_perf.md) 작성 후 승인받아 개선.

### 변경 요약
- `JsonStore`가 파일을 최초 1회만 읽어 인메모리 캐시(`self._cache`)로 보관하고,
  조회는 캐시에서 처리(깊은 복사로 반환하여 호출자가 캐시를 오염시키지 않도록 함).
  쓰기는 여전히 매번 즉시 디스크에 반영(영속성 계약 불변).

### 성능 재측정 결과
| 작업 | 캐싱 전 | 캐싱 후 |
|---|---|---|
| 시료 200종 등록 | 0.84s | 0.15s (5.6배) |
| 주문 1000건 접수 | 13.89s | 5.11s (2.7배) |
| 승인/거절 1000건 | 17.77s | 18.06s (개선 없음) |
| 생산처리 651건 | 12.62s | 12.96s (개선 없음) |

### 잔여 한계 (사용자 승인 하에 범위 제한)
쓰기(`insert`/`update`/`delete`)는 매번 파일 전체를 재직렬화하는 구조라 캐싱으로 해결되지
않음. 배치 쓰기 등 추가 최적화는 실사용 규모(수십~수백 건)에서 불필요하다고 판단하여
진행하지 않기로 결정하고 CLAUDE.md에 알려진 한계로 명시함.

### TDD 사이클 기록
- RED: 외부 파일 변조 시나리오 테스트 작성 → 캐싱 미구현 상태에서 실패 확인
- GREEN: 캐싱 구현 후 3개 신규 테스트 PASS, 전체 73/73 PASS(회귀 없음)
- REVIEW: 스트레스 테스트 재실행으로 읽기 성능 개선과 정확성(음수 재고 0) 재확인

### 요청사항 충족 여부 체크리스트
- [x] 읽기 성능 개선 (캐싱)
- [x] 쓰기 계약(즉시 디스크 반영) 불변 유지, 회귀 없음
- [x] 캐시 오염 방지(반환값은 복사본) 테스트로 검증
- [x] 잔여 성능 한계 문서화(CLAUDE.md)

### 변경된 파일 목록
- `app/repositories/json_store.py` (수정)
- `tests/test_json_store_cache.py` (신규)
- `docs/design/phase10_perf.md` (신규)
- `CLAUDE.md`, `docs/PLAN.md` (수정)

## Phase 11: CI 자동화 / JsonStore 배치 쓰기 / MainController 테스트 보강

### 요청받은 작업
자체 채점 결과("90~92점")에서 지적된 3가지 감점 요인을 해소: (1) CI 자동화 부재,
(2) JsonStore 쓰기 성능 한계 미해결, (3) View(MainController 메뉴 분기) 테스트 커버리지 부재.
사용자가 AskUserQuestion에서 "CI + 성능 + View 테스트 보강까지 전부" 진행을 승인함.

### 해석
- CI: 로컬 `scripts/verify.py`만으로는 "커밋 전 사람이 직접 실행해야만" Test Verify가
  보장되므로, 원격(GitHub Actions)에서도 동일한 pytest가 자동 실행되도록 안전망을 추가한다.
- 성능: Phase 10에서 의도적으로 미룬 쓰기 성능은, 매 호출 즉시 flush하는 안전 계약을 깨지
  않으면서 "flush=False + 명시적 flush()"를 선택적으로 제공하는 방식으로 해결한다. 인터랙티브
  콘솔 흐름(사람이 1건씩 처리)은 계속 즉시 flush하므로 동작 변화가 없고, 대량 삽입/배치
  시나리오에서만 실질적 이득을 얻는다(정직하게, "1000건 승인 반복 호출" 같은 실사용과 거리가
  먼 인위적 벤치마크 자체의 체감 속도가 바뀌는 것은 아님).
- View 테스트: `MainController`는 생성자에서 리포지토리를 직접 생성해 파일 경로가 고정되어
  있으므로, 프로덕션 코드를 건드리지 않고 `monkeypatch.chdir` + `builtins.input` 큐 monkeypatch로
  테스트 가능함을 확인하고 그대로 적용했다(테스트 편의를 위한 불필요한 의존성 주입 리팩토링은
  하지 않음).

### 변경 요약
1. `.github/workflows/test.yml` 추가 — push/PR 시 `pip install -r requirements.txt` +
   `pytest tests/ -v` 자동 실행.
2. `app/repositories/json_store.py`: `insert`/`update`/`delete`에 `flush: bool = True` 파라미터
   추가, `flush()` 메서드 신규 추가(dirty flag 방식). 외부 인터페이스 기본 동작(즉시 flush)은
   불변.
3. `tests/test_json_store_cache.py`: flush=False 시 디스크 미반영, `flush()` 호출 시 일괄 반영,
   update/delete도 flush=False를 받는지 검증하는 테스트 4건 추가.
4. `tests/test_main_controller.py` 신규 — `MainController.run()` 및 6개 서브메뉴의 정상/오류
   분기 14개 테스트 추가. `main_controller.py` 커버리지 0% → 100%.
5. `docs/design/phase11.md` 신규, `docs/PLAN.md`/`docs/design/README.md`에 Phase 11 반영.

### 성능 검증 결과 (배치 삽입 1000건, `time.perf_counter()` 측정)
| 방식 | 소요 시간 |
|---|---|
| 기본값(매번 flush, 기존과 동일) | 1.51s |
| `flush=False` + 마지막 `flush()` 1회 | 0.0047s (반올림하면 0.0s로 보여 과장처럼 비칠 수 있어 소수점 4자리로 기록) |

### 커버리지 결과
전체 테스트 73개 → 90개(json_store 4건 + main_controller 14건 신규 반영, 회귀 없음).
`app/` 전체 라인 커버리지 94% → 99%. `main_controller.py` 0% → 100%.
`console_view.py`는 92%로 유지(순수 출력 포맷팅 일부만 미커버, 기존 설계 결정 유지).

### TDD 사이클 기록
- RED: `test_insert_with_flush_false_does_not_write_immediately` 등 4개 테스트 작성 →
  `TypeError: unexpected keyword argument 'flush'`로 실패 확인
- GREEN: `JsonStore`에 `flush` 파라미터와 `flush()` 메서드 구현 → 4개 신규 테스트 PASS,
  기존 76개 포함 전체 PASS
- RED: `test_main_controller.py` 작성 시점에는 아직 `MainController` 코드 변경이 없었으므로
  이 파트는 순수 테스트 추가(기존 동작을 검증하는 회귀 방지 테스트)이며 RED 단계가 존재하지
  않음(신규 기능이 아니라 기존 기능의 커버리지 보강이기 때문)
- REVIEW: 커버리지 리포트로 `main_controller.py` 100%, 전체 90/90 PASS 재확인

### 요청사항 충족 여부 체크리스트
- [x] CI 워크플로우 추가 (GitHub Actions)
- [x] JsonStore 쓰기 배치 옵션 추가, 기존 계약(즉시 flush 기본값) 불변
- [x] 배치 삽입 성능 개선 실측(1.51s → 0.0047s)
- [x] MainController 메뉴 분기 테스트 추가, 100% 커버리지
- [x] 회귀 없음 (90/90 PASS)

### 변경된 파일 목록
- `.github/workflows/test.yml` (신규)
- `app/repositories/json_store.py` (수정)
- `tests/test_json_store_cache.py` (수정)
- `tests/test_main_controller.py` (신규)
- `docs/design/phase11.md` (신규)
- `docs/PLAN.md`, `docs/design/README.md` (수정)
